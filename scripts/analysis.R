...
for (geom_type in Geometries) {
  ...
  for(p_range in 1:nrow(cent_ranges)){
    ...
    if (geom_type == "G") {
      ...
      Pulse <- amplitude_pulse(Pulse$density_val, amp=1)
    ...
    if (geom_type== "G"){
      path1 <- paste(paste0("Outputs/sd",noise_sd_values,"/", geom_type), "GeometryFWHM", cent_ranges$Percentage[p_range],
                     "NoiseFWHM", noise_fwhm, methods[1], sep = "_")
      path2 <- paste(paste0("Outputs/sd",noise_sd_values,"/", geom_type), "GeometryFWHM", cent_ranges$Percentage[p_range],
                     "NoiseFWHM", noise_fwhm, methods[2], sep = "_")
    } else {
      path1 <- paste(paste0("Outputs/sd",noise_sd_values,"/", geom_type), "GeometryPercentage", cent_ranges$Percentage[p_range],
                     "NoiseFWHM", noise_fwhm, methods[1], sep = "_")
      path2 <- paste(paste0("Outputs/sd",noise_sd_values,"/", geom_type), "GeometryPercentage", cent_ranges$Percentage[p_range],
                     "NoiseFWHM", noise_fwhm, methods[2], sep = "_")
    }
