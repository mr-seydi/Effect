...
###############################################################
data_1 <- read_excel("results/Simulation1_dense1001.xlsx")
data_3 <- read_excel("results/Simulation_SqP1_dense1001.xlsx")
data_9 <- read_excel("results/Simulation_GP1_dense1001.xlsx")
################################################################
################################################################
left<-centered_ranges(seq(5, 100, by = 5), domain=c(0,500))
right<-centered_ranges(seq(5, 100, by = 5), domain=c(500,1000))
c_ranges <- cbind(left,right)
Pulse_two_square <- matrix(0, nrow = 1001 , ncol = nrow(c_ranges))
for (i in 1:nrow(c_ranges)){
  one_side <- square_pulse(start_end_pulse=c(left$Start[i],left$End[i]), start_height=0,
                           pulse_height=1, continuum_size=1001)
...
Pulse_long_two_square <- data.frame(Domain = seq(0,100,length.out=1001), Pulse_two_square) %>%
...
##############################################
effect1 <- centered_ranges (seq(5, 100, by = 5),domain=c(0,1000))
Pulse <- matrix(0, nrow = 1001 , ncol = nrow(effect1))
for (i in 1:nrow(effect1)){
  Pulse[,i] <- square_pulse (start_end_pulse=c(effect1$Start[i],effect1$End[i]),
                             start_height=0, pulse_height=1, continuum_size=1001)
}
# Example: Assuming Pulse is a matrix with columns representing different curves
# Convert to data frame and reshape
Pulse_long <- data.frame(Domain = seq(0,100,length.out=1001), Pulse) %>%
  pivot_longer(-Domain, names_to = "Curve", values_to = "Effect")
# Add a curve order column based on the column order in the matrix
...
Pulse_G <- matrix(0, nrow = 1001 , ncol = length(Pfwhm))
for (i in 1:length(Pfwhm)){
  P_G <- gaussian_pulse(center=500, fwhm=Pfwhm[i], continuum_size=1001)
...
# Convert to data frame and reshape
Pulse_long_GP <- data.frame(Domain = seq(0,100,length.out=1001), Pulse_G) %>%
  pivot_longer(-Domain, names_to = "Curve", values_to = "Effect")
# Add a curve order column based on the column order in the matrix
...
ggsave("results/Plots/plot_dense_1001.jpeg", final_plot, width = 190, height = 196, units = "mm", dpi = 600)
