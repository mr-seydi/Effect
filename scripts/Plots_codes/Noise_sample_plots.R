...
NFWHM_10_curve <- Noise_plot(NFWHM_10_1)
NFWHM_30_curve <- Noise_plot(NFWHM_30_1)
NFWHM_50_curve <- Noise_plot(NFWHM_50_1)
...
SP_signal_plot <- pulse_single_plot(signal_data=SP, legend_name="5%", legend_title="Domain")

P2S_signal_plot <- pulse_single_plot(signal_data=Pulse_two_square, legend_name="50%", legend_title="Domain")

PG_signal_plot <- pulse_single_plot(signal_data=P_G, legend_name="100%", legend_title="SFWHM")

  

SP_plot <- sample_plot(data_type = "baseline", Org_data = NULL,
                       Signal_curve = SP, noise1_data = NFWHM_10_1,
                        noise2_data = NFWHM_10_2,Title = "", legend = "none")
P2S_plot <- sample_plot(data_type = "baseline", Org_data = NULL,
                        Signal_curve = Pulse_two_square, noise1_data = NFWHM_30_1,
                        noise2_data = NFWHM_30_2,Title = "", legend = "none")
PG_plot <- sample_plot(data_type = "baseline", Org_data = NULL,
                       Signal_curve = P_G, noise1_data = NFWHM_50_1,
                       noise2_data = NFWHM_50_2,Title = "", legend = "none")
...
column_labels <- arrangeGrob(
  textGrob("Noise", gp = gpar(fontsize = 12, fontface = "bold"), hjust = -0.6),  # Adjust horizontally to the left
  textGrob("Geometry", gp = gpar(fontsize = 12, fontface = "bold"), just = "centre"),     # Default position (centered)
  textGrob("Sample Curves", gp = gpar(fontsize = 12, fontface = "bold"), just = "centre"), # Adjust horizontally to the right
  ncol = 3
)
...
ggsave("results/Plots/Noise_Sample.jpeg", final_plot, width = 190, height = 196, units = "mm", dpi = 600)
