...
for (i in 1:length(Pfwhm)){
  ...
  P_G <- amplitude_pulse(data=P_G$density_val, amp=0.5)
  Pulse_G[,i] <- P_G
}
# Convert to data frame and reshape
...
PS_1_SPM <- power_plot_SPM(data_7,Title="")
PS_1_SnPM <- power_plot_SnPM(data_7,Title="")
PS_3_SPM <- power_plot_SPM(data_8,Title="")
PS_3_SnPM <- power_plot_SnPM(data_8,Title="")
PS_9_SPM <- power_plot_SPM(data_11,Title="")
PS_9_SnPM <- power_plot_SnPM(data_11,Title="")
...
legend_plot <- power_plot_SPM(data_7, Title = "", legend = "bottom")
...
ggsave("results/Plots/plot_finalhalf.jpeg", final_plot, width = 190, height = 196, units = "mm", dpi = 600)
