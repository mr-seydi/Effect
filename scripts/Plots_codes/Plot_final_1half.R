...
for (i in 1:nrow(effect1)){
  Pulse[,i] <- square_pulse (start_end_pulse=c(effect1$Start[i],effect1$End[i]),
                             start_height=0, pulse_height=1.5)
}
...
for (i in 1:length(Pfwhm)){
  ...
  P_G <- amplitude_pulse(data=P_G$density_val, amp=1.5)
  Pulse_G[,i] <- P_G
}
# Convert to data frame and reshape
...
PS_1_SPM <- power_plot_SPM(data_5,Title="")
PS_1_SnPM <- power_plot_SnPM(data_5,Title="")
PS_3_SPM <- power_plot_SPM(data_6,Title="")
PS_3_SnPM <- power_plot_SnPM(data_6,Title="")
PS_9_SPM <- power_plot_SPM(data_10,Title="")
PS_9_SnPM <- power_plot_SnPM(data_10,Title="")
...
legend_plot <- power_plot_SPM(data_5, Title = "", legend = "bottom")
...
ggsave("results/Plots/plot_final1half.jpeg", final_plot, width = 190, height = 196, units = "mm", dpi = 600)
